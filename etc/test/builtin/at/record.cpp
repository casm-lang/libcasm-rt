...
TEST( libcasm_rt__builtin_at, record )
{
    const auto integerType = libstdhl::Memory::get< IntegerType >();
    const auto booleanType = libstdhl::Memory::get< BooleanType >();

    const auto recordType = libstdhl::Memory::make< RecordType >(
        Types( { integerType, booleanType, integerType } ),
        std::vector< std::string >{ "a", "b", "c" } );

    const std::unordered_map< std::string, Constant > values = { { "a", IntegerConstant( -123 ) },
                                                                 { "b", BooleanConstant( true ) },
                                                                 { "c", IntegerConstant( 321 ) } };

    auto recordConstant = RecordConstant( recordType, values );

    std::vector< Constant > arg;
    Constant res;

    arg = { recordConstant, IntegerConstant( 1 ) };
    libcasm_rt::Value::execute(
        id,
        RelationType( integerType, Types( { recordType, integerType } ) ),
        res,
        arg.data(),
        arg.size() );
    EXPECT_TRUE( res == IntegerConstant( -123 ) );

    arg = { recordConstant, IntegerConstant( 2 ) };
    libcasm_rt::Value::execute(
        id,
        RelationType( booleanType, Types( { recordType, integerType } ) ),
        res,
        arg.data(),
        arg.size() );
    EXPECT_TRUE( res == BooleanConstant( true ) );

    arg = { recordConstant, IntegerConstant( 3 ) };
    libcasm_rt::Value::execute(
        id,
        RelationType( integerType, Types( { recordType, integerType } ) ),
        res,
